...
def attach_parts(*, target, parts, uploaded_by, html_body: str = "", max_bytes: int | None = None):
    """Create Ticket/CommentAttachment rows for each non‑inline part.

    ``target`` is a Ticket or Comment instance. Parts larger than ``max_bytes``
    are skipped so one huge attachment never blocks the ticket. Returns
    ``(created, skipped)`` where ``skipped`` is a list of ``(filename, size)``.
    """
    ...
    for p in parts or []:
        ...
        if len(p.data) > max_bytes:
            skipped.append((p.filename or "attachment.bin", len(p.data)))
            continue
